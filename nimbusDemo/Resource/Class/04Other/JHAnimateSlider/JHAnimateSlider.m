...
#define kCheckColorGreen			RGBCOLOR_HEX(0x39d167)
...
@interface JHAnimateSlider ()

@property (nonatomic, strong) UIImageView* loadingCircle;
@property (nonatomic, strong) JHLoadingMaskView* loadingMask;
...
@implementation JHAnimateSlider
...
- (void)setUpApperance{
    ...
    [self setThumbImage:[UIImage imageNamed:@"jh_slider_round.png"] forState:UIControlStateNormal];
...
- (void)creatViews
{
    _loadingCircle = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"jh_slider_loading_circle_alpha.png"]];
    ...
    // arrow icon
    UIImageView* arrowIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"jh_slider_arrow.png"]];
    ...
    if (!_sliderRound){
        for (UIView* view in self.subviews){
            if ([view isKindOfClass:[UIImageView class]]){
                ...
                if ([imageView.image isEqual:[UIImage imageNamed:@"jh_slider_round.png"]]
                    || [imageView.image isEqual:[[UIImage imageNamed:@"jh_slider_round_finish.png"] imageWithTintColor:kCheckColorGreen]]
                    ){
...
- (void)setSliderState:(SliderState)sliderState{
    ...
    if (sliderState == kSliderStateFinished){
        [self setThumbImage:[[UIImage imageNamed:@"jh_slider_round_finish.png"] imageWithTintColor:kCheckColorGreen]
                   forState:UIControlStateNormal];
        _loadingCircle.hidden = YES;
    }else if (sliderState == kSliderStateUnFinished){
        [self setThumbImage:[UIImage imageNamed:@"jh_slider_round.png"]
                   forState:UIControlStateNormal];
...
- (JHLoadingMaskView *)loadingMask{
    if (!_loadingMask){
        _loadingMask = [[JHLoadingMaskView alloc] initWithFrame:CGRectZero];
    }
    return _loadingMask;
}
...
@interface JHLoadingMaskView ()

@property (nonatomic,strong) CADisplayLink *displayLink;  //是一个能让我们以和屏幕刷新率相同的频率将内容画到屏幕上的定时器

@property (nonatomic) CGFloat animateValueInterval;  //动画时间间隔

@end

@implementation JHLoadingMaskView
