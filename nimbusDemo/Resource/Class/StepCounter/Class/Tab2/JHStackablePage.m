...
@implementation JHStackablePage
...
- (void)updateVisiblePageswWithContentOffset:(CGFloat)offset {
    ...
    NIDPRINT(@"index:%ld, %ld", (long)startIdx, (long)endIdx);
    if (startIdx != oldStartIdx || endIdx != oldEndIdx) {
        // move to resue
        // update visibleViews;
        for (NSInteger i = oldStartIdx; i < startIdx; ++i) {
            // remove pages at the begining
            [self popFrontVisiblePage];
        }
        for (NSInteger i = oldEndIdx - 1; i >= endIdx; --i) {
            // remove pages at the end
            [self popBackVisblePage];
        }
        
        for (NSInteger i = MAX(oldEndIdx, startIdx); i < endIdx; ++i) {
            // add pages
            [self addPageAtIndex:i fromBottom:YES];
        }
        
        for (NSInteger i = MIN(oldStartIdx - 1, endIdx - 1); i >= startIdx; --i) {
            // add pages
    ...
    for (NSInteger i = 0; i < self.visiblePages.count; ++i) {
        StackablePage *page = self.visiblePages[i];
        page.positionInVisiblePages = NSMakeRange(i, self.visiblePages.count);
    }
    
    NIDPRINT(@"%lu, %lu", (unsigned long)self.visiblePages.count, (unsigned long)self.reusablePages.count);
}
...
- (void)updateStackLayout:(CGFloat)offset {
    ...
    if (self.visiblePages.count > 0) {
        ...
        CGFloat minYOffset = self.topView.height + self.scrollView.contentOffset.y;
        if (self.pin) {
            yOffset = MAX(minYOffset, yOffset);
        }
        ...
        NIDPRINT(@"y: %f", yOffset);
        
        yOffset += stackedHeight + addedSpace - overlapHeight;
        
        for (NSInteger i = 1; i < self.visiblePages.count; ++i) {
            NIDPRINT(@"y: %f", yOffset);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // headerView相关事件
    if (scrollView.isDragging && -offset < self.topView.height) {
    ...
    else if (scrollView.isDragging && -offset >= self.topView.height) {
