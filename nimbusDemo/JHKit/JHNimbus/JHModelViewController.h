...
@interface JHModelViewController : JHViewController<JHModelDelegate>
{
    @protected
    struct {
...
//
// 默认为YES, 如果设置为YES, 则在Model加载，结束过程中会调用 ShowLoading/ShowError等
//
@property (nonatomic) BOOL showDefaultLoadingStatus;
...
//
// JH_HTTP_PARSE_BLOCK 容易导致内存泄露，因此导入此函数，避免Block过多地引用成员变量等
//
