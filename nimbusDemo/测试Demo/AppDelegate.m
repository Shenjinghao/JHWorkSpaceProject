...
- (void) customizeNavigationBar {
    // 设置样式:
    // 只修改不同的NavigationBar, 其他地方，例如视频播放器则不修改
    
    id appearance = [UINavigationBar appearanceWhenContainedIn:[UINavigationController class], nil];
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] < 7.0) {
        ...
        [appearance setBarTintColor: COLOR_A1];
    }
...
- (JHTabBarController *)tabbarController
{
    if ([self.window.rootViewController isKindOfClass:[UINavigationController class]]) {
        return nil;
    }else{
        JHIndexViewController *VC = (JHIndexViewController *)self.window.rootViewController;
        return [VC rootTabbarController];
    }
}

- (JHNavigationController *)navigationController
{
...
- (JHViewController *)topViewController
{
    if ([self.window.rootViewController isKindOfClass:[UINavigationController class]]) {
        UINavigationController *naVC = (UINavigationController *)self.window.rootViewController;
        return (JHViewController *)naVC.topViewController;
    }else{
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "chunyu.__Demo" in the application's documents directory.
